...
import com.groupmeeting.view.expense.ExpenseListDialog;
import com.groupmeeting.view.place.FinalPlaceEditDialog;
import com.groupmeeting.view.place.PlaceListDialog;
import com.groupmeeting.view.place.PlaceVoteDialog;
import com.groupmeeting.view.round.RoundListDialog;
import com.groupmeeting.view.schedule.FinalDateEditDialog;
import com.groupmeeting.view.schedule.FinalTimeEditDialog;
import com.groupmeeting.view.schedule.MyScheduleEditDialog;
import com.groupmeeting.view.schedule.ScheduleInputDialog;
import com.groupmeeting.view.schedule.ScheduleResultDialog;
import com.groupmeeting.view.travel.TravelScheduleEditDialog;
import com.groupmeeting.view.travel.TravelScheduleResultDialog;
...
import com.groupmeeting.repository.ExpenseRepository;
...
public class RoomDetailView extends JFrame {
    ...
    /** 뒤로가기, 방 이름, 방 코드, 참여 인원, 마이페이지를 한 줄에 표시합니다. */
    private JPanel buildTopBar() {
        ...
        JLabel codeLabel = new JLabel("방 코드 " + room.getCode());
        codeLabel.setOpaque(true);
        codeLabel.setBackground(Color.WHITE);
        codeLabel.setForeground(Theme.PRIMARY_GREEN_DARK);
        codeLabel.setFont(new Font("맑은 고딕", Font.PLAIN, 11));
        codeLabel.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(Theme.BORDER_GREEN, 1, true),
                BorderFactory.createEmptyBorder(3, 7, 3, 7)
        ));

        JButton memberButton = new JButton(room.getMemberCount() + "명");
        memberButton.setFont(new Font("맑은 고딕", Font.BOLD, 11));
        memberButton.setForeground(Theme.PRIMARY_GREEN_DARK);
        memberButton.setBackground(Theme.ACCENT_GREEN);
        memberButton.setBorder(BorderFactory.createEmptyBorder(4, 8, 4, 8));
        memberButton.setFocusPainted(false);
        memberButton.setCursor(new Cursor(Cursor.HAND_CURSOR));
        memberButton.setToolTipText("참여자 명단 확인");
        memberButton.addActionListener(e -> handleShowMemberList());

        infoPanel.add(nameLabel);
        infoPanel.add(Box.createHorizontalStrut(8));
        infoPanel.add(codeLabel);
        infoPanel.add(Box.createHorizontalStrut(8));
        infoPanel.add(memberButton);
    ...
    private void ensurePlaceListSeeded() {
        placeRepository.seedIfEmpty(
                room.getCode(),
                availabilityRepository.getLegacyPlaceSuggestions(room.getCode())
        );
    }
