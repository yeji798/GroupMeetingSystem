...
public class ScheduleInputDialog extends JDialog {
    ...
    private void initComponents() {
        ...
        Theme.alignAsCenteredColumn(
                dateLabel, calendarPanel,
                timeLabel, timeRow,
                placeLabel, placeField,
                listLabel, entriesScrollPane,
                submitButton
        );
    ...
    private void handleSubmit() {
        ...
        if (!place.isBlank()) {
            boolean alreadyExists = placeRepository.getPlaces(room.getCode()).stream()
                    .anyMatch(existing -> existing.equalsIgnoreCase(place));
            if (!alreadyExists && !placeRepository.addPlace(room.getCode(), place)) {
                showWarning("추천 장소를 저장하지 못했습니다. 다시 시도해 주세요.");
                return;
            }
        }
